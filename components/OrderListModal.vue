<template>
  ...
  >
    <div class="overflow-y-auto max-h-[55vh] space-y-4">
      ...
      >
        <Icon
          ...
          class="h-16 w-16 mx-auto mb-4 text-slate-600"
        />
        ...
        <div class="flex flex-col lg:flex-row lg:items-center gap-4">
          ...
          <div class="flex items-center space-x-4 lg:flex-1">
            <img
              ...
              class="w-16 h-10 rounded-md bg-slate-600 object-cover"
            />
            <div class="flex-1">
              <h3 class="font-semibold text-white text-sm">
                {{ order.productName }}
              </h3>
              <p class="text-slate-300 text-xs">{{ order.game }}</p>
              <div class="flex items-center space-x-2 mt-1">
                <span
                  class="text-xs bg-slate-600 text-slate-200 px-2 py-1 rounded"
                  >{{ order.server }}</span
                >
                <span class="text-xs text-slate-400">UID: {{ order.uid }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="lg:flex-1 space-y-2">
            <div class="flex justify-between items-center">
              ...
              <span class="font-bold text-amber-400">${{ order.amount }}</span>
            </div>
            <div class="flex justify-between items-center">
    ...
    <template #footer>
      <!-- 分页器 -->
      <div v-if="totalPages > 1" class="border-t border-slate-600 pt-4">
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center space-x-2">
            ...
            <button
              ...
              class="p-2 rounded-lg border border-slate-600 text-slate-300 hover:text-white hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              @click="prevPage"
            >
              ...
              <button
                ...
                :class="[
                  ...
                  page === currentPage
                    ...
                    : 'border border-slate-600 text-slate-300 hover:text-white hover:bg-slate-700',
                ]"
                @click="goToPage(page)"
              >
                {{ page }}
      ...
      <!-- 无分页时的订单统计 -->
      <div v-else class="border-t border-slate-600 pt-4 text-center">
